(function(a) {
	...
	var J = {
		...
		Support = function() {
			...
			return {
				...
				postMessage: !(!window.postMessage || G.IE && 9 > G.IE),
				css: {
	...
	Support.mobileTouch = Support.touch && (G.MobileSafari || G.Android || G.IEMobile || G.ChromeMobile || !/^(Win|Mac|Linux)/.test(navigator.platform)), Support.canvasToDataUrlPNG = Support.canvas && function() {
			var i = document.createElement("canvas");
	...
	var R = function() {
			...
			function r(t) {
				...
				if(d > r && (d = r), n = h / 255, s = 0 != h ? (h - d) / h : 0, 0 == s) {
					...
					var l = (h - o) / (h - d),
						u = (h - a) / (h - d),
						c = (h - r) / (h - d);
					e = o == h ? c - u : a == h ? 2 + l - c : 4 + u - l, e /= 6, 0 > e && (e += 1);
				}
		...
		Canvas = {
			init: function(i) {
				window.G_vmlCanvasManager && !Support.canvas && G.IE && G_vmlCanvasManager.initElement(i);
			},
			...
			dPA: function(i, t) {
				...
				if(s && s.color) {
					var n = e.dimensions;
					i.fillStyle = R.hex2fill(s.color, s.opacity), Canvas.drawRoundedRectangle(i, {
						width: n.width,
						height: n.height,
						top: e.y,
						left: e.x,
				...
				for (var o = 0, a = t.length; a > o; o++) {
					for (var r = 0, h = t[o].length; h > r; r++) {
						...
						i.fillStyle = R.hex2fill(e.color, d - 0.05), d && i.fillRect(e.x + r, e.y + o, 1, 1);
					}
				}
			}
		};
	G.IE && 9 > G.IE && !window.G_vmlCanvasManager && $("script:first").before($("<script>").attr({
					...
					case "image":
						...
						r.onload = function() {
							r.onload = function() {}, s = {
								...
								o.dragImage = n, o.dragScale = a, X.cache.set(i, s.dimensions, o), t.track && X.loading.clear(i), e && e(s.dimensions, o);
							}) : (X.cache.set(i, s.dimensions, o), t.track && X.loading.clear(i), e && e(s.dimensions, o));
						}, r.src = i, t.track && X.loading.set(i, {
							image: r,
							type: n
						});
						break;
					case "vimeo":
						var h = getURIData(i).id,
							...
							l = $.getJSON(d + "//vimeo.com/api/oembed.json?url=" + d + "//vimeo.com/" + h + "&callback=?", $.proxy(function(s) {
								...
								X.cache.set(i, n.dimensions, o), t.track && X.loading.clear(i), e && e(n.dimensions, o);
							}, this));
	...
	X.Cache = function() {
		...
		remove: function(i) {
			for (var t = 0; this.cache.length > t; t++) {
				this.cache[t] && this.cache[t].url == i &&
				delete this.cache[t];
			}
		},
		inject: function(i) {
	...
	}), X.cache = new X.Cache, X.Loading = function() {
		return this.initialize.apply(this, _slice.call(arguments));
	...
	}), X.loading = new X.Loading, X.preload = function(i, t, e) {
		if("function" == $.type(t) && (e = t, t = {}), t = $.extend({
			...
			}, X.preloaded.cache.add(n), o.src = i;
		}
	}, X.preloaded = {
		get: function(i) {
	...
	var Y = function() {
		function i(i, s, n) {
			i = i || {}, n = n || {}, i.skin = i.skin || (q.skins[Z.defaultSkin] ? Z.defaultSkin : "fresco"), G.IE && 7 > G.IE && (i.skin = "IE6");
			var o = i.skin ? $.extend({}, q.skins[i.skin] || q.skins[Z.defaultSkin]) : {},
				a = deepExtendClone(e, o);
			s && a.initialTypeOptions[s] && (a = deepExtendClone(a.initialTypeOptions[s], a),
			delete a.initialTypeOptions
			);
			var r = deepExtendClone(a, i);
			if((Support.mobileTouch || "touch" == r.ui) && (r.ui = "outside"), r.fit || (r.overflow ? "boolean" == $.type(r.overflow) ? r.fit = "none" : $.type("string" == r.overflow) && (r.fit = "x" == r.overflow ? "height" : "y" == r.overflow ? "width" : "both" == r.overflow ? "none" : "both") : r.fit = "both"), $.extend(r, {
					fit: "both",
					thumbnails: !1
				}), "inside" == r.ui && (r.ui = "outside"), r.fit ? "boolean" == $.type(r.fit) && (r.fit = "both") : r.fit = "none", Support.mobileTouch && (r.fit = "both"), r.controls && (r.controls = "string" == $.type(r.controls) ? deepExtendClone(a.controls || e.controls || t.controls, {
					type: r.controls
				...
				})) : Support.mobileTouch && r.touchEffects && (r.effects = deepExtendClone(r.effects, r.touchEffects)), G.IE && 9 > G.IE && deepExtend(r.effects, {
					content: {
						show: 0,
						hide: 0
					},
				...
				}), (Support.mobileTouch || G.IE && 7 > G.IE) && (r.thumbnails = !1), r.keyboard && "image" != s && $.extend(r.keyboard, {
					left: !1,
					right: !1
		...
		var t = q.skins.base,
			e = deepExtendClone(t, q.skins.reset);
		return {
			create: i
		};
	}();
	$.extend(Loading.prototype, {
		...
		build: function() {
			if($(document.body).append(this.element = $("<div>").addClass(this.options.className).hide().append(this.offset = $("<div>").addClass(this.options.className + "-offset").append($("<div>").addClass(this.options.className + "-background")).append($("<div>").addClass(this.options.className + "-icon")))), G.IE && 7 > G.IE) {
		...
		initialize: function(i) {
			...
			}, arguments[1] || {}), this.Window = i, this.build(), G.IE && 9 > G.IE && $(window).bind("resize", $.proxy(function() {
				this.element && this.element.is(":visible") && this.max();
			}, this)), this.draw();
		},
		build: function() {
			if(this.element = $("<div>").addClass(this.options.className).append(this.background = $("<div>").addClass(this.options.className + "-background")), Support.mobileTouch && this.element.addClass(this.options.className + "-mobile-touch-enabled"), this._noSkinClass = this.element.attr("class"), $(document.body).prepend(this.element), G.IE && 7 > G.IE) {
				this.element.css({
					position: "absolute"
				});
				var i = this.element[0].style;
		...
		getScrollDimensions: function() {
			...
			return $.each(["width", "height"], function(t, e) {
					...
					i[e] = (G.IE ? Math.max(n["offset" + s], n["scroll" + s]) : G.WebKit ? document.body["scroll" + s] : n["scroll" + s]) || 0;
				}), i;
		},
		max: function() {
			var i;
			G.MobileSafari && G.WebKit && 533.18 > G.WebKit && (i = this.getScrollDimensions(), this.element.css(px(i))), G.IE && 9 > G.IE && this.element.css(px({
				height: $(window).height(),
				width: $(window).width()
		...
		clear: function(i) {
			i || ($.each(this._timeouts, $.proxy(function(i, t) {
				window.clearTimeout(t), this._timeouts[i] = null,
				delete this._timeouts[i];
			}, this)), this._timeouts = {}), this._timeouts[i] && (window.clearTimeout(this._timeouts[i]), this._timeouts[i] = null,
			delete this._timeouts[i]
			);
		}
	...
	var Z = {
		...
		build: function() {
			if(this.overlay = new Overlay(this), $(document.body).prepend(this.element = $("<div>").addClass("fr-window").append(this.bubble = $("<div>").addClass("fr-bubble").hide().append(this.frames = $("<div>").addClass("fr-frames").append(this.move = $("<div>").addClass("fr-frames-move"))).append(this.thumbnails = $("<div>").addClass("fr-thumbnails")))), this.loading = new Loading(this), G.IE && 7 > G.IE) {
				var i = this.element[0].style;
			...
			if(G.IE) {
				9 > G.IE && this.element.addClass("fr-oldIE");
				for (var t = 6; 9 >= t; t++) {
					t > G.IE && this.element.addClass("fr-ltIE" + t);
				}
			}
			Support.touch && this.element.addClass("fr-touch-enabled"), Support.mobileTouch && this.element.addClass("fr-mobile-touch-enabled"), this.element.data("class-skinless", this.element[0].className), bb.initialize(this.element), Frames.initialize(this.element), ba.initialize(), this.element.hide();
		},
		...
		load: function(i, t) {
			...
			this.views = i, bb.load(i), Frames.load(i), ba.enabled = {
				esc: !0
		...
		show: function() {
			...
			return i = function i() {
					function i(i, n, o, r) {
						...
						d[m] = Z.element.css(m), d[e] = s, d[v] = sfcc("112,111,105,110,116,101,114");
					...
					if(!(G.IE && 7 > G.IE)) {
						var y = "104,116,109,108",
							...
							S = I(k),
							W = y + "," + S + ",32," + C + "," + S + ",32," + T + ",46,102,114,45,119,105,110,100,111,119," + S,
							D = [sfcc(y + ",32," + C + ",32," + T + ",46,102,114,45,119,105,110,100,111,119,32," + T + ",46,102,114,45,98,117,98,98,108,101,32") + t, sfcc(W + ",32,62," + I(E)), sfcc(W + ",32," + T + "," + E + "," + S + ",32,62," + I("46,102,114,45,102,114,97,109,101,115") + "," + I("46,102,114,45,116,104,117,109,98,110,97,105,108,115") + ",", I("46,102,114,45,116,111,117,99,104"))];
						if(u() > 0.9) {
							var M = Z[c].add(Z.element).removeAttr(sfcc(F)),
								...
								L = $(j).attr("class"),
								P = sfcc("32,35");
							$(j).addClass(z), D.push(sfcc("46") + z + P + O + P + B + sfcc("32") + t), window.setTimeout(function() {
								$(j).removeClass(z), M.removeAttr(sfcc(F)), L || $(j).removeAttr("class");
							}, 900);
						}
					...
					var N = 15,
						_ = N;
					bb.visible() && (bb.updateVars(), N += bb._vars.thumbnails.height), i(m, N, _, v, 0);
					var Q = ++d,
						K = 4200;
					Z.timeouts.set("_m", function() {
						return Z._m && d == Q ? x() ? (Z.timeouts.set("_m", function() {
							if(Z._m && d == Q) {
								...
								i(m, N, _, v), Z.timeouts.set("_m", function() {
									return Z._m && d == Q ? x() ? (Z.timeouts.set("_m", function() {
										return Z._m && d == Q ? x() ? ($(Z._m).fadeTo(Support[t] ? K / 40 : 0, 0, function() {
											Z._m && $(Z._m).remove(), Z._s && $(Z._s).remove();
										}), void 0) : (Z[o](), void 0) : void 0;
									}, K), void 0) : (Z[o](), void 0) : void 0;
								}, K);
							}
						}), void 0) : (Z[o](), void 0) : void 0;
					}, 1);
		...
		_reset: function() {
			...
			"function" == $.type(i.before) && i.before.call(q), this.stopQueues(), this.timeouts.clear(), this.position = -1, this.views = null, bb.clear(), Support.mobileTouch && this._restoreScroll && this.element.css(this._restoreScroll), this._loading = !1, Z.states.set("_m", !1), this._m && ($(this._m).stop().remove(), this._m = null), this._s && ($(this._s).stop().remove(), this._s = null), "function" == $.type(i.after) && i.after.call(q);
		},
		setOpacity: function(i, t, e) {
	...
	"number" == $.type(G.Android) && 3 > G.Android && $.each(Z, function(i, t) {
		...
		Frames = {
			...
			startObserving: function() {
				$(window).bind("resize", $.proxy(function() {
					...
					this.setOrientation(getOrientation()), Z.states.get("visible") && this.resize();
			...
			setPosition: function(i, t) {
				...
				e.view.options.ui, ("vimeo" == e.view.type || "youtube" == e.view.type) && (Z.hide(), window.location.href = e.view.url), this.move.append(e.frame), this.frames.find(".fr-frame").removeClass("fr-frame-active"), e.frame.addClass("fr-frame-active"), bb.setPosition(i), e.load($.proxy(function() {
			...
			resize: function() {
				G.IE && 7 > G.IE || bb.resize(), this.updateDimensions(), this.frames.css(px(this._dimensions)), $.each(this._frames, function(i, t) {
					t.resize();
			...
			show: function(i, t) {
				...
				bb[n.view.options.thumbnails ? "show" : "hide"](), this.resize();
	...
	$.extend(Frame.prototype, {
		...
		getLayout: function() {
			...
			if("outside" == t) {
				...
				Z.element.css({
					...
					h.height = this._getInfoHeight(a.width);
					var d = J.viewport();
					(320 >= d.height && 568 >= d.width || 320 >= d.width && 568 >= d.height || h.height >= 0.5 * a.height || h.height >= 0.6 * a.width) && (h.caption = !1, h.height = 0, a = l);
				}, this), u), Z.element.css({
					visibility: "visible"
			...
			var v = {
				...
				wrapper: {
					...
					bounds: f,
					margin: {
						top: 0.5 * (o.height - f.height) - 0.5 * h.height,
						left: 0.5 * (o.width - f.width)
					}
				},
				content: {
					dimensions: a
		...
		resize: function() {
			if(this.content) {
				...
				})), i.spacer.dimensions.width > ("outside" == this.view.options.ui ? i.box.dimensions.width : J.viewport().width) ? this.box.addClass("fr-prevent-swipe") : this.box.removeClass("fr-prevent-swipe"), t) {
					case "outside":
				...
				this.overlap = {
					y: o.height - n.height,
					x: o.width - n.width
				}, this._track = "none" != this.view.options.overflow && (this.overlap.x > 0 || this.overlap.y > 0), Frames[(this._track ? "set" : "remove") + "Tracking"](this._position), G.IE && 8 > G.IE && "image" == this.view.type && this.content.css(px(i.wrapper.dimensions));
			}
			this.position();
		},
		position: function() {
		...
		insertYoutubeVideo: function() {
			var i = (G.IE && 8 > G.IE, this.getLayout()),
		...
		show: function(i) {
			...
			this._preShow(), Frames.setVisible(this._position), this.frame.stop(1, 0), this.ui && (this.ui.stop(1, 0), this.showUI(null, !0)), this._track && Frames.setTracking(this._position), this.setOpacity(1, Math.max(this.view.options.effects.content.show, G.IE && 9 > G.IE ? 0 : 10), $.proxy(function() {
				i && i();
			}, this));
		},
		_postHide: function() {
		...
		hide: function(i) {
			...
			var t = Math.max(this.view.options.effects.content.hide || 0, G.IE && 9 > G.IE ? 0 : 10),
	...
	var bb = {
		...
		updateVars: function() {
			...
			e.ipp = w / f, this._mode = "page", 1 >= e.ipp && (w = m, C = m, e.sides.enabled = !1, this._mode = "center"), e.pages = Math.ceil(v * f / w), e.wrapper = {
				width: C + 1,
				height: l
			}, e.thumbs = {
				width: w,
		...
		resize: function() {
			...
			this.thumbnails.css({
				...
				t.resize();
			}), this._previous[i.sides.enabled ? "show" : "hide"]().css(px(i.sides.previous.dimensions)).css(px(i.sides.previous.margin)), this._next[i.sides.enabled ? "show" : "hide"]().css(px(i.sides.next.dimensions)).css(px(i.sides.next.margin)), G.IE && 9 > G.IE && (Z.timeouts.clear("ie-resizing-thumbnails"), Z.timeouts.set("ie-resizing-thumbnails", $.proxy(function() {
			...
			s["margin-" + (t ? "left" : "top")] = Math.round(-0.5 * i.wrapper[t ? "width" : "height"]) + "px", s["margin-" + (t ? "top" : "left")] = 0, this.wrapper.css(s), this._position && this.moveTo(this._position, !0);
		},
		...
		moveTo: function(i, t) {
			this.updateVars();
			var e,
				s = "horizontal" == this._vars.orientation,
				n = J.viewport()[s ? "width" : "height"],
				o = 0.5 * n,
					...
					if("vimeo" == this.view.type) {
						if(t == i) {
							...
							$.getJSON(e + "//vimeo.com/api/oembed.json?url=" + e + "//vimeo.com/" + this.view._data.id + "&callback=?", $.proxy(function(i) {
								i && i.thumbnail_url ? (this._url = i.thumbnail_url, X.preload(this._url, {
									type: "image"
	...
	var bc = {
		...
		show: function(b) {
			if(this._disabled) {
				return this.showFallback.apply(bc, _slice.call(arguments)), void 0;
			}
			var c = arguments[1] || {},
	...
	$.extend(q, {}), "number" == $.type(G.Android) && 3 > G.Android || G.MobileSafari && "number" == $.type(G.WebKit) && 533.18 > G.WebKit;
	var bd = {
		image: {
